...
gboolean has_geometry = FALSE, single_click = FALSE;
...
void init(void)
{
	...
	if (!icon_name)
		icon_name = g_strdup_printf("%s/wmxmms.xpm", DATADIR);
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "hg:s:c:i:nv", long_options, &o)) != -1)
	{
		switch (c)
		{
			case 0:
