/*
 * refuse.c - Send/View authorization refused message
 */
...
/*----------------------------------------------------------------------------------------------------*/
 ...
 MSGMGR icqmsg_refuse = {       sizeof(MSGMGR),
                                MSG_REFUSED,
                                MSG_NORMAL,                  /* No reply for this kind of message         */
...
/*----------------------------------------------------------------------------------------------------*/
 ...
 static const char * _System title(HICQ icq, HUSER usr, ULONG uin, BOOL out, ICQMSG *msg, char *buffer, int sz)
 {
    ...
    strcpy(buffer, out ? "Refuse authorization to " : "Authorization refused from ");
 ...
 static int _System send(const MSGEDITHELPER *dlg, MSGEDITWINDOW hwnd, HICQ icq, ULONG uin)
 {
    ...
    if(!text)
    {
       icqWriteSysLog(icq,PROJECT,"Memory allocation error when trying to send authorization refused message");
       return -1;
    }

    dlg->getString(hwnd,ICQMSGC_TEXT,MAX_MSG_SIZE,text);

    icqSendMessage(icq, uin, MSG_REFUSED, text);

    if(usr && (usr->flags & USRF_TEMPORARY) )
    {
       icqRemoveUser(icq, usr);
       sprintf(text,"User %s (ICQ#%lu) removed from database",icqQueryUserNick(usr),uin);
